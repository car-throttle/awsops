...
helper.deleteObjects = function (config, opts, callback) {
  ...
  var s3 = (config instanceof AWS.S3) ? config : (new AWS.S3(config || {}));
...
helper.lookupEc2Instances = function (config, opts, callback) {
  ...
  var ec2 = (config instanceof AWS.EC2) ? config : (new AWS.EC2(config || {}));
...
helper.listDirectoryOnS3 = function (opts, callback) {
  opts = opts || {};
  if (!opts.bucket) return callback(new Error('Missing bucket from uploadFileToS3 opts'));
...
helper.uploadFileToS3 = function (opts, callback) {
  opts = opts || {};
  if (!opts.bucket) return callback(new Error('Missing bucket from uploadFileToS3 opts'));
